...
class DbManager private constructor(context: Context) {
    ...
    fun tableOpenInformation(tableName: String, columns:Array<String> = arrayOf("*"), orderBy:String, dateFrom: String? = null, dateTo: String? = null ): ArrayList<ArrayList<String>> {

        var otherTableName = ""
        var columnWithId = ""
        var columnDate = ""
        //41-61 making sql query
        when (tableName) {
            DataBase.TABLE_OUTCOME_NAME -> {
                ...
                columnDate = DataBase.COLUMN_OUTCOME_DATE
            }
            DataBase.TABLE_INCOME_NAME -> {
                ...
                columnDate = DataBase.COLUMN_INCOME_DATE
            }
        }

        var sql = "SELECT ${columns.joinToString()} FROM $tableName LEFT OUTER JOIN " +
                "$otherTableName ON $tableName.$columnWithId=$otherTableName._id "

        if (dateFrom != null && dateTo != null)
            sql += "WHERE $columnDate <= '$dateTo' AND $columnDate >= '$dateFrom' "
